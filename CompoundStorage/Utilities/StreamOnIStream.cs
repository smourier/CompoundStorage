...
{
    public sealed class StreamOnIStream : Stream
    {
        ...
        public StreamOnIStream(IStream stream, bool releaseOnDispose)
        {
            _stream = stream;
        ...
        public STGC? CommitOnDispose { get; set; }
        public void Flush(STGC options) => CheckDisposed().Commit((int)options);
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (stream != null)
            {
                if (CommitOnDispose.HasValue)
                {
                    try
                    {
                        stream.Commit((int)CommitOnDispose.Value);
                    }
                    catch
                    {
                        //+ do nothing
